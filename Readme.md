...
In order to cross-compile and generate the u-dma-buf.ko module, an example of a proper args set to run the makefile with:

make ARCH=arm64 KERNEL_SRC=../../build_tools/sd_boot_script/sd_boot_script/linux-xlnx/ CONFIG_U_DMA_BUF_KMOD_TEST=m CONFIG_U_DMA_BUF_MGR=m

Note that KERNEL_SRC= the local directory of the linux-xlnx/ repo that was used to generate the target Kernel image. Please change that to 
suit your local directory.
